...
{
    // S’assure que le dossier Scripts existe
    EnsureScriptsDirectoryExists(state_);

    const path scriptsDirectory = state_.root / "Scripts";

    // ✅ Récupère l’arbre des scripts via ScriptUtils (refactor)
    ...
    std::vector<TreeNodeData> scriptTree = ScriptUtils::BuildGuiTree(scriptRoots, scriptInfoMap);
    ...
    // Prépare le baseName
    String nameInputStr(scriptName_);
    String trimmedInput = TrimCopy(nameInputStr);
    ...
    // Validation du nom
    String rawName = TrimCopy(String(scriptName_));
    if (rawName.IsEmpty())
    {
        LogAndStoreError(scriptError_, "Script name cannot be empty.", false);
        return;
    }
    if (ContainsPathSeparator(rawName))
    {
        LogAndStoreError(scriptError_, "Script name cannot contain path separators.", false);
        return;
    }
    ...
    {
        LogAndStoreError(scriptError_, "A file with this name already exists.", false);
        return;
    }
    ...
    if (dirErr)
    {
        ...
        msg += dirErr.message();
        LogAndStoreError(scriptError_, std::move(msg));
        return;
    }
    ...
    {
        LogAndStoreError(scriptError_, "Failed to create the header file.");
        return;
    }
    ...
    if (!sourceFile.is_open())
    {
        ...
        LogAndStoreError(scriptError_, "Failed to create the source file.");
    ...
    if (!headerGood || !sourceGood)
    {
        ...
        LogAndStoreError(scriptError_, "Failed to write the script files to disk.");
        return;
    }
