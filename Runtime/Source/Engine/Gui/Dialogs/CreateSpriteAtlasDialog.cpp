...
CreateSpriteAtlasDialog::CreateSpriteAtlasDialog(ContentBrowserState& state, String& selectedEntry)
    : ModalDialog(state, selectedEntry, "ContentBrowserCreateSpriteAtlas"),
      columns_(0), rows_(0), frameRate_(24.0f), loop_(true)
{
    atlasName_[0] = '\0';
    atlasError_.Clear();
    framesDir_.clear();
...
{
    String nameStr = TrimCopy(String(atlasName_));
    if (nameStr.IsEmpty())
    {
        LogAndStoreError(atlasError_, "Atlas name cannot be empty.", false);
        return false;
    }
    ...
    {
        LogAndStoreError(atlasError_, "An atlas or image with this name already exists.", false);
        return false;
    }

    if (!AtlasGenerator::GenerateAtlas(framesDir_, columns_, rows_, 0, 0, frameRate_, loop_))
    {
        LogAndStoreError(atlasError_, "Failed to generate atlas (check for PNG files with matching sizes).", false);
        return false;
    }
