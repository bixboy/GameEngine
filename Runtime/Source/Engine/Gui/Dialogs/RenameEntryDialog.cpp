...
void RenameEntryDialog::DrawContent()
{
    ...
    // --- Input Field ---
    bool renameTriggered = InputTextWithLabel("New name", renameBuffer_, IM_ARRAYSIZE(renameBuffer_),
    ...
    // --- Validate Input ---
    String newNameStr = TrimCopy(String(renameBuffer_));
    if (newNameStr.IsEmpty()) {
        LogAndStoreError(renameError_, "Name cannot be empty.", false);
        return;
    }

    if (ContainsPathSeparator(newNameStr)) {
        LogAndStoreError(renameError_, "Name cannot contain path separators.", false);
        return;
    }

    if (!isScriptGroup_ && targetPath_.empty()) {
        LogAndStoreError(renameError_, "No entry selected for rename.", false);
        return;
    }
...
bool RenameEntryDialog::HandleScriptRename(const String& newNameStr)
{
    ...
    if (headerOld.empty() && sourceOld.empty())
        return LogAndStoreError(renameError_, "No entry selected for rename.", false), false;
    ...
    if (newBaseName.empty())
        return LogAndStoreError(renameError_, "Name cannot be empty.", false), false;
    ...
    if ((!newHeader.empty() && fs::exists(newHeader)) || (!newSource.empty() && fs::exists(newSource)))
        return LogAndStoreError(renameError_, "An entry with this name already exists.", false), false;
    ...
    for (auto& [oldP, newP] : toRename)
    {
        ...
        if (ec)
        {
            ...
            LogAndStoreError(renameError_, msg);
...
bool RenameEntryDialog::HandleSingleRename(const String& newNameStr)
{
    ...
    if (oldPath.empty())
        return LogAndStoreError(renameError_, "Invalid target path.", false), false;
    ...
    if (!TryRename(oldPath, newPath, renameError_))
        return false;
