...
namespace BixEngine::resources
{
    ...
    std::vector<SpriteFrame> SpriteAtlasUtils::GenerateFrames(Texture& texture, int columns, int rows, int padding, int margin)
    {
        ...
        const float cellWidth = (texWidth - 2.0f * static_cast<float>(margin) - static_cast<float>((columns - 1) * padding)) / static_cast<float>(columns);
        const float cellHeight = (texHeight - 2.0f * static_cast<float>(margin) - static_cast<float>((rows - 1) * padding)) / static_cast<float>(rows);
