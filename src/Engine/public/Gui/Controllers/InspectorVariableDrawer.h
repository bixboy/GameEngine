...
{
  inline void DrawExposedVariablesSection(BaseAssetEditorController::SharedState &state, std::string_view sectionLabel, const char * /*tableId*/, const char *emptyMessage = "No exposed variables.")
  {
    ...
    for (std::size_t index = 0; index < state.exposedVariables.size(); ++index) {
      ...
      const auto drawFloat = [&]() {
        ...
        if (Widgets::DrawDragControl("##ExposedValue", value, 0.1f, nullptr, nullptr, "%.3f"))
        {
          char formatted[64]{};
      ...
      const auto drawVector = [&](int dimension)
      {
        ...
        const bool edited =
            dimension == 2 ? ImGui::DragFloat2("##ExposedValue", values, 0.1f) : ImGui::DragFloat3("##ExposedValue", values, 0.1f);
      ...
      else if (matchesType(typeName, "int") || matchesType(typeName, "int32_t") || matchesType(typeName, "std::int32_t"))
      {
        drawInt();
        handled = true;
      }
