...
namespace BixEngine::Gui::ActorInspector
{
    ...
    bool DrawSupportedProperty(const Bix::Reflection::PropertyInfo& property, void* instance, const std::string& label)
    {
        ...
        // Int
        if (ExposedVariableUtils::MatchesType(property.TypeName, "int") || ExposedVariableUtils::MatchesType(property.TypeName, "int32_t") || ExposedVariableUtils::MatchesType(property.TypeName, "std::int32_t"))
        {
            int& v = property.Get<int>(instance);
            bool changed = Widgets::DrawDragControl("##value", v, 1.0f, nullptr, nullptr, "%d");
            (void)changed;
        ...
        if (ExposedVariableUtils::MatchesType(property.TypeName, "float"))
        {
            ...
            bool changed = Widgets::DrawDragControl("##value", v, 0.1f, nullptr, nullptr, "%.3f");
            (void)changed;
        ...
        // Vector2
        if (ExposedVariableUtils::MatchesType(property.TypeName, "Math::Vector2") || ExposedVariableUtils::MatchesType(property.TypeName, "Vector2"))
        {
        ...
        // Vector3
        if (ExposedVariableUtils::MatchesType(property.TypeName, "Math::Vector3") || ExposedVariableUtils::MatchesType(property.TypeName, "Vector3"))
        {
        ...
        // String types
        if (ExposedVariableUtils::MatchesType(property.TypeName, "String") || ExposedVariableUtils::MatchesType(property.TypeName, "std::string"))
        {
    ...
    bool DrawReflectedProperty(const Bix::Reflection::PropertyInfo& property, void* instance)
    {
        ...
        if (!handled)
        {
            ...
            ImGui::TextUnformatted((label + " : " + (typeName.empty() ? "Unsupported" : typeName)).c_str());
        }
    ...
    static void GatherClassPropertiesImpl(const Bix::Reflection::ClassInfo& cls, std::vector<const Bix::Reflection::PropertyInfo*>& out)
    {
        if (cls.SuperClass)
            GatherClassPropertiesImpl(*cls.SuperClass, out);
    ...
    bool DrawClassProperties(const Bix::Reflection::ClassInfo& classInfo, void* instance, bool includeHeader, const char* headerLabel, bool showEmptyMessage)
    {
        ...
        GatherClassPropertiesImpl(classInfo, props);
        ...
        if (includeHeader && headerLabel && headerLabel[0]) Utils::DrawSeparatorText(headerLabel);
    ...
    // -------------------------
    // Exposed variable section
    // -------------------------
    void DrawExposedVariablesSection(BaseAssetEditorController::SharedState& state, std::string_view sectionLabel, const char* emptyMessage)
    {
        ...
        for (size_t i = 0; i < state.exposedVariables.size(); ++i)
        {
            ...
            else if (ExposedVariableUtils::MatchesType(typeName, "float"))
            {
                ...
                {
                    char buf[64]; std::snprintf(buf, sizeof(buf), "%.3ff", v);
                    var.value = buf;
                }
            ...
            // vec2
            else if (ExposedVariableUtils::MatchesType(typeName, "Vector2") || ExposedVariableUtils::MatchesType(typeName, "Math::Vector2"))
            {
                float v[2] = {
            ...
            // vec3
            else if (ExposedVariableUtils::MatchesType(typeName, "Vector3") || ExposedVariableUtils::MatchesType(typeName, "Math::Vector3"))
            {
                float v[3] = {
                    nums.size() > 0 ? nums[0] : 0.f,
                    nums.size() > 1 ? nums[1] : 0.f,
