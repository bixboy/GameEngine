...
const fs::path kGeneratedOutputDir = fs::path("build") / os::host() / os::arch() / "debug" / "Intermediate" / "GeneratedHeaders";
...
int main(int argc, char** argv)
{
    ...
    fs::remove_all(kGeneratedOutputDir, cleanupErr);
    if (cleanupErr)
    {
        std::cerr << "[BixHeaderTool] Warning: failed to clean " << kGeneratedOutputDir << "\n";
    }

    // ✅ Crée le dossier s’il n’existe pas encore
    std::error_code createErr;
    fs::create_directories(kGeneratedOutputDir, createErr);
    if (createErr)
    {
        std::cerr << "[BixHeaderTool] Error: cannot create output dir " << kGeneratedOutputDir << "\n";
        return 2;
    }
    ...
    for (const auto& header : headers)
    {
        ...
        // ✅ Pas de sous-dossier : tout à plat
        fs::path generatedPath = fs::path(kGeneratedOutputDir) / (header.stem().string() + ".generated.h");
