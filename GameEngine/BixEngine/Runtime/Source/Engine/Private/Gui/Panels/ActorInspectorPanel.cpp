...
namespace BixEngine::Gui
{
    namespace
    {
        ...
        void DrawActorOverview(Game::Actor& actor, ActorInspectorState& state)
        {
            ...
            if (ImGui::BeginChild("OverviewCard", ImVec2(0.0f, 0.0f), false, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_AlwaysUseWindowPadding))
            {
                ...
                if (ImGui::BeginTable("ActorOverviewTable", 2, ImGuiTableFlags_SizingStretchProp | ImGuiTableFlags_PadOuterX | ImGuiTableFlags_NoSavedSettings))
                {
                    ...
                    ImGui::TableSetColumnIndex(1);
                    const ImVec4 componentColor = ImVec4(0.28f, 0.44f, 0.72f, 1.0f);
                    const ImVec4 componentTextColor = ImVec4(0.95f, 0.97f, 1.0f, 1.0f);
                    DrawBadge(componentLabel.c_str(), componentColor, componentTextColor);
